...
test.describe('IDEE.layer.WMC', () => {
  ...
  test('Add layer', async ({ page }) => {
    await page.evaluate(() => {
      const wmc_001 = new IDEE.layer.WMC({
        url: `${IDEE.config.RECURSOS_ESTATICOS}/Datos/WMC/wmc_grupos.xml`,
        name: 'Secciones',
      });
  ...
  test('Add two WMC', async ({ page }) => {
    await page.evaluate(() => {
      const wmc_001 = new IDEE.layer.WMC({
        url: `${IDEE.config.RECURSOS_ESTATICOS}/Datos/WMC/satelite.xml`,
        name: 'Satelite',
      });
      window.wmc_001 = wmc_001;
      ...
      const wmc_002 = new IDEE.layer.WMC({
        url: `${IDEE.config.RECURSOS_ESTATICOS}/Datos/WMC/context_cdau_hibrido_25830_no_cache.xml`,
        name: 'Híbrido',
      });
