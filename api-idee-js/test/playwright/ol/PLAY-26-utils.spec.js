...
test.describe('IDEE.Utils', () => {
  ...
  test('Comprobando funcionalidades de utils', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      IDEE.utils.classToggle(elemento, 'clase_1');
    });
    const listaclases = await page.getAttribute('body > p', 'class');
    expect.soft(listaclases).toContain('clase_1'); // comprueba que añade la nueva clase
    ...
    expect.soft(listaclases2).not.toContain('clase_1'); // comprueba que elimina la clase ya existente
    ...
    const styleGeometryCheck = await page.evaluate(() => {
